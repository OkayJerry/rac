...
function useLocalStorage() {
  console.log("[HOOK] useLocalStorage: Initializing.");
  const [entries, setEntries] = reactExports.useState([]);
  reactExports.useEffect(() => {
    console.log("[HOOK] useLocalStorage: useEffect running. Getting initial entries and adding listener.");
    chrome.storage.local.get([STORAGE_KEY], (result) => {
      console.log("[HOOK] useLocalStorage: Initial data fetched from storage.", result);
      if (Array.isArray(result[STORAGE_KEY])) {
        setEntries(result[STORAGE_KEY]);
      } else {
        console.log("[HOOK] useLocalStorage: No entries found, initializing storage.");
        chrome.storage.local.set({ [STORAGE_KEY]: [] });
      }
    });
    const storageListener = (changes, areaName) => {
      console.log("[HOOK] useLocalStorage: storageListener detected changes.", { areaName, changes });
      if (areaName === "local" && changes[STORAGE_KEY]) {
        console.log("[HOOK] useLocalStorage: Entries changed, updating state.");
        setEntries(changes[STORAGE_KEY].newValue || []);
      }
    };
    chrome.storage.onChanged.addListener(storageListener);
    return () => {
      console.log("[HOOK] useLocalStorage: useEffect cleanup. Removing storage listener.");
      chrome.storage.onChanged.removeListener(storageListener);
    };
  }, []);
  const persist = reactExports.useCallback(
    (newEntries) => new Promise((resolve) => {
      console.log("[HOOK] persist: Persisting new entries to storage.", { count: newEntries.length });
      chrome.storage.local.set({ [STORAGE_KEY]: newEntries }, () => {
        console.log("[HOOK] persist: Successfully set to storage.");
  ...
  const addEntry = reactExports.useCallback(
    async (text, vector) => {
      console.log("[HOOK] addEntry: Attempting to add new entry.", { text: text.substring(0, 50) + "..." });
      const normalized = text.trim();
      const { [STORAGE_KEY]: currentEntries = [] } = await chrome.storage.local.get(STORAGE_KEY);
      const existing = currentEntries.find(
        (e) => e.text.trim() === normalized
      );
      if (existing) {
        console.log("[HOOK] addEntry: Entry with the same text already exists.", { id: existing.id });
        return existing;
      }
      const newEntry = {
      ...
      console.log("[HOOK] addEntry: Creating new entry.", { id: newEntry.id });
      const updated = [...currentEntries, newEntry];
      await persist(updated);
      console.log("[HOOK] addEntry: Persisted new entry.");
      chrome.storage.local.getBytesInUse(null, (bytes) => {
        const usedMB = bytes / (1024 * 1024);
        console.log(`[HOOK] addEntry: Checking storage quota. Used: ${usedMB.toFixed(2)}MB`);
        if (usedMB > 8 && !window.__racQuotaWarned) {
          console.warn("[HOOK] addEntry: Storage usage exceeds 80%. Triggering notification.");
          window.__racQuotaWarned = true;
          chrome.notifications.create({
            type: "basic",
            iconUrl: "icons/icon48.png",
            title: "Clipboard storage nearly full",
            message: "You’ve used over 80% of the allotted 10 MB. Grant unlimitedStorage?",
            buttons: [{ title: "Grant unlimited" }]
          });
        }
      });
      return newEntry;
  ...
  const updateEntry = reactExports.useCallback(
    async (entry) => {
      console.log("[HOOK] updateEntry: Updating entry.", { id: entry.id });
      const { [STORAGE_KEY]: currentEntries = [] } = await chrome.storage.local.get(STORAGE_KEY);
      const updated = currentEntries.map(
        (e) => e.id === entry.id ? entry : e
      );
      await persist(updated);
      console.log("[HOOK] updateEntry: Persisted updated entries.");
    },
    [persist]
  );
  const removeEntry = reactExports.useCallback(
    async (id) => {
      console.log("[HOOK] removeEntry: Removing entry.", { id });
      const { [STORAGE_KEY]: currentEntries = [] } = await chrome.storage.local.get(STORAGE_KEY);
      const updated = currentEntries.filter(
        (e) => e.id !== id
      );
      await persist(updated);
      console.log("[HOOK] removeEntry: Persisted entries after removal.");
    },
    [persist]
  );
  return { entries, addEntry, updateEntry, removeEntry };
}
function EditableSelection({ text, onChange }) {
...
function SuggestionList({ suggestions, onCopy }) {
  if (suggestions.length === 0) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "No suggestions yet." });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { style: { listStyle: "none", padding: 0 }, children: suggestions.map(({ id, text, score }) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "li",
    {
      style: {
        display: "flex",
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { title: `Score: ${score.toFixed(4)}`, children: text }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: () => onCopy(text), children: "Copy" })
      ]
    },
    id
  )) });
...
function EntryForm({ onSave }) {
  console.log("[Component] EntryForm: Rendering.");
  const [text, setText] = reactExports.useState("");
  const handleSave = () => {
    console.log("[Component] EntryForm: handleSave triggered.");
    if (text.trim()) {
      console.log("[Component] EntryForm: Text is valid, calling onSave.");
      onSave(text);
      setText("");
    } else {
      console.warn("[Component] EntryForm: Save attempt with empty text.");
    }
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: handleSave, style: { width: "100%" }, children: "Save" })
  ] });
}
function MainView() {
  console.log("[POPUP] MainView: Component rendering.");
  const [selection, setSelection] = reactExports.useState("Select text on the page to begin.");
  const [suggestions, setSuggestions] = reactExports.useState([]);
  const { addEntry } = useLocalStorage();
  reactExports.useEffect(() => {
    console.log("[POPUP] MainView: useEffect mounting. Adding message listener.");
    const messageListener = (msg) => {
      console.log("[POPUP] MainView: Message listener received message:", msg);
      if (msg.type === "search-results") {
        console.log("[POPUP] MainView: Received search-results, updating state.");
        setSelection(msg.payload.original);
        setSuggestions(msg.payload.suggestions);
      }
      if (msg.type === "embedding-result" && msg.payload.textToSave) {
        console.log("[POPUP] MainView: Received embedding-result, adding new entry.");
        addEntry(msg.payload.textToSave, msg.payload.vector);
      }
    };
    chrome.runtime.onMessage.addListener(messageListener);
    return () => {
      console.log("[POPUP] MainView: useEffect cleanup. Removing message listener.");
      chrome.runtime.onMessage.removeListener(messageListener);
    };
  }, [addEntry]);
  reactExports.useEffect(() => {
    console.log("[POPUP] MainView: useEffect mounting to get last selection.");
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      const tabId = tabs[0]?.id;
      if (tabId !== void 0) {
        console.log(`[POPUP] MainView: Active tab found (${tabId}). Sending get-last-selection.`);
        chrome.runtime.sendMessage({
          type: "get-last-selection",
          payload: { tabId }
        });
      } else {
        console.warn("[POPUP] MainView: Could not get active tab ID.");
      }
    });
  }, []);
  const handleSelectionChange = (newText) => {
    console.log("[POPUP] MainView: handleSelectionChange called.");
    setSelection(newText);
  };
  const handleSave = (text) => {
    console.log("[POPUP] MainView: handleSave called. Sending get-embedding message.");
    chrome.runtime.sendMessage({
      type: "get-embedding",
      payload: { text }
    });
  ...
  const handleCopy = (text) => {
    console.log("[POPUP] MainView: handleCopy called.");
    navigator.clipboard.writeText(text);
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { padding: "16px", width: "300px" }, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "Your Selection" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(EditableSelection, { text: selection, onChange: handleSelectionChange }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("hr", {}),
    /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "Suggestions" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SuggestionList, { suggestions, onCopy: handleCopy }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("hr", {}),
    /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "Create New Entry" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(EntryForm, { onSave: handleSave })
  ] });
}
chrome.runtime.onMessage.addListener((msg) => {
  if (msg.type === "bg-debug-log") console.log("[BG→PAGE]", msg.payload);
});
const container = document.getElementById("root");
clientExports.createRoot(container).render(/* @__PURE__ */ jsxRuntimeExports.jsx(MainView, {}));
